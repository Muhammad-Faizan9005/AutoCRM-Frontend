...
const buildTitle = (c) => { const l = String(c||'').split('\n')[0]; return (!l ? 'Untitled' : l.length > 60 ? l.slice(0,60)+'...' : l); };
const splitContent = (c) => { const [f,...r] = String(c||'').split('\n'); return { title: f||'', body: r.join('\n').trim() }; };
const mapNote = (n, uid) => ({ id: n.id, title: buildTitle(n.content), content: n.content, author: !n.author_id ? 'Unknown' : (uid && String(n.author_id)===String(uid)) ? 'You' : String(n.author_id).slice(0,8), time: formatDate(n.updated_at||n.created_at) });
...
const Notes = ({ user }) => {
  ...
  const fetchNotes = useCallback(async (skip=0, append=false) => {
    ...
    try {
      ...
      const data = await apiFetch(`/api/notes/?skip=${skip}&limit=${lim}`);
      if (rid !== latestRequestId.current) return;
      const m = data.map(n => mapNote(n, uid));
      if (append) setNotes(p => [...p, ...m]); else setNotes(m);
      setTotalLoaded(p => p + m.length);
      setHasMore(m.length === lim);
    } catch (err) { if (rid === latestRequestId.current) setError(err?.message || 'Unable to load notes.'); }
    finally { if (rid === latestRequestId.current) { if (!append) setLoading(false); else setIsLoadingMore(false); } }
  }, [uid]);

  useEffect(() => { fetchNotes(0, false); }, [fetchNotes]);

  const filtered = useMemo(() => notes.filter(n => n.title.toLowerCase().includes(searchTerm.toLowerCase())), [notes, searchTerm]);

  const openEdit = (note) => { const { title, body } = splitContent(note.content); setSelectedNote(note); setFormData({ title, content: body }); setModalType('edit'); };
  const openCreate = () => { setFormData({ title: '', content: '' }); setModalType('create'); };

  const handleDelete = async (id) => {
    if (!window.confirm('Delete this note?')) return;
    try { await apiFetch(`/api/notes/${id}`, { method: 'DELETE' }); setNotes(p => p.filter(n => n.id !== id)); } catch (e) { setError(e?.message || 'Delete failed.'); }
  };
  ...
  const handleSave = async (e) => {
    ...
    if (!uid) { setError('User not identified.'); return; }
    setIsSaving(true);
    try {
      const created = await apiFetch('/api/notes/', { method: 'POST', body: JSON.stringify({ entity_type: DEFAULT_ENTITY_TYPE, entity_id: uid, content: combined }) });
      setNotes(p => [mapNote(created, uid), ...p]); setModalType(null);
    } catch (e) { setError(e?.message || 'Save failed.'); } finally { setIsSaving(false); }
  };

  const handleUpdate = async (e) => {
    e.preventDefault(); if (!selectedNote?.id) return; setError('');
    const combined = [formData.title.trim(), formData.content.trim()].filter(Boolean).join('\n');
    if (!combined) { setError('Content required.'); return; }
    setIsSaving(true);
    try {
      const updated = await apiFetch(`/api/notes/${selectedNote.id}`, { method: 'PATCH', body: JSON.stringify({ content: combined }) });
      setNotes(p => p.map(n => n.id === selectedNote.id ? mapNote(updated, uid) : n)); setModalType(null);
    } catch (e) { setError(e?.message || 'Update failed.'); } finally { setIsSaving(false); }
  };
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={() => fetchNotes(0,false)} className="btn btn-ghost btn-icon" aria-label="Refresh"><RotateCcw size={16}/></button>
        ...
        ) : filtered.length === 0 ? <EmptyState type="notes"/> : (
            ...
            {filtered.map((note, idx) => (
              <motion.div key={note.id} className={`card ${NOTE_BORDERS[idx%3]}`} style={{ padding: 16, cursor: 'pointer' }} onClick={() => openEdit(note)} whileHover={{ scale: 1.01 }} initial={{ opacity:0,y:6 }} animate={{ opacity:1,y:0 }}>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:12, paddingTop:10, borderTop:'1px solid var(--color-border)' }}>
                  <span style={{ fontSize:'var(--text-xs)', color:'var(--color-text-tertiary)' }}>{note.author} · {note.time}</span>
        ...
        <AnimatePresence>
          {modalType && (
            <motion.div className="modal-overlay" initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }} onClick={() => setModalType(null)}>
              <motion.div className="modal-content" style={{ maxWidth:480 }} onClick={e => e.stopPropagation()} initial={{ opacity:0,scale:0.97,y:4 }} animate={{ opacity:1,scale:1,y:0 }} exit={{ opacity:0,scale:0.97 }}>
                <div style={{ display:'flex', justifyContent:'space-between', padding:'16px 20px', borderBottom:'1px solid var(--color-border)' }}>
                  ...
                  <button onClick={() => setModalType(null)} className="btn btn-ghost btn-icon" aria-label="Close"><X size={18}/></button>
                </div>
                <form onSubmit={modalType==='create' ? handleSave : handleUpdate} style={{ padding:20, display:'flex', flexDirection:'column', gap:14 }}>
                  ...
                  <div style={{ display:'flex', justifyContent:'flex-end' }}><button type="submit" disabled={isSaving} className="btn btn-primary">{isSaving ? 'Saving...' : modalType==='create' ? 'Create' : 'Update'}</button></div>
                </form>
              </motion.div>
            </motion.div>
          )}
